...
def block_results(
    ...
    """
    Display the Participant's block results; Presiona la barra espaciadora para continuar; ESC para alternar pantalla completa; 5 segundos de espera
    ...
    def draw():
        ...
        _render_centered_text(
            screen, font,
            f"Bloque {block} Completado! Rendimiento:",
            screen_rect.centery - 75,
            cfg.COCO_RGB,
        )

        _render_centered_text(
            screen, font,  f"Precisión: {accuracy}%", screen_rect.centery - 20, cfg.COCO_RGB
        )
        _render_centered_text(
            screen, font, f"Velocidad Promedio: {avg_RT} s", screen_rect.centery + 20, cfg.COCO_RGB
        )
        pygame.display.flip()
    draw()

    logger.info(f"Precisión: {accuracy:.3}% , Velocidad Promedio: {avg_RT:.3} s")
